...
void betweenness(int matrizAntecessores[][MAXNUMVERTICES], int numVertices){
    int valorBetweenness[numVertices]; //cada lugar do vetor armazena o valor de betweenness para cada vertice
    int cidadeAtual, j, k, seguidorDeCaminho;

    for (j = 0; j < numVertices; j++){
        valorBetweenness[j] = 0;
    }

    for(cidadeAtual = 0; cidadeAtual < numVertices; cidadeAtual++){
        for(j = 0; j < numVertices; j++){
            if(j != cidadeAtual){
                for(k = 0; k < numVertices; k++){
                    if(j != k){
                        seguidorDeCaminho = matrizAntecessores[j][k];
                        while(seguidorDeCaminho != cidadeAtual){
                            if(seguidorDeCaminho == cidadeAtual){
                                valorBetweenness[cidadeAtual]++;
                                seguidorDeCaminho = cidadeAtual;
                            }
                            else{
                                seguidorDeCaminho = matrizAntecessores[j][seguidorDeCaminho];
                            }
    ...
    for(j = 0; j < numVertices; j++){
        printf("Betweenness da cidade %d: %d", j, valorBetweenness[j]);
    }
}
